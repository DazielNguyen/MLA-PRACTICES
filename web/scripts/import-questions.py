...
def legacy_bank():
    ...
    for q in records:
        ...
        for number, image in enumerate(q['images'], 1):
            ...
            destination = OUTPUT / 'images' / filename
...
def write_exports(bank):
    ...
    for q in bank:
        ...
        for letter, text in q['choices'].items():
            md += [f'{letter}. {text}']
            md += [f"![{im['alt']}]({image_base}{Path(im['url']).name})" for im in q['images'] if im['slot'] == letter]
        analysis = q['analysis']
...
def main():
    ...
    localized = localize([q for q in study_bank if q['collection'] == 'mla'])
    web_bank = [q for q in localized if q.get('origin') != 'original']
    (OUTPUT / 'ORIGINAL_ARCHIVE.json').write_text(json.dumps(
        [q for q in localized if q.get('origin') == 'original'], ensure_ascii=False, separators=(',', ':')) + '\n')
    ...
    # Publish complete conditional keys as scored source answers, not a separate review pool.
    # Retain the caveats and legacy unscored-attempt migration without claiming AWS verification.
    for q in web_bank:
        if q['status'] == 'review':
            q['status'] = 'source'
    ...
    # Retired banks keep validation shapes only, so existing progress remains readable.
    shapes = [[q['id'], ''.join(q['choices']), q['required'], q['status'] == 'review'] for q in old + originals]
    (APP / 'src/data/retired-question-shapes.ts').write_text(
        '// Generated by import-questions.py. Validation metadata only; no question text or answers.\n'
        + 'export const retiredQuestionShapes: [number, string, number, boolean][] = '
        + json.dumps(shapes, separators=(',', ':')) + ';\n')
    for image in (APP / 'public/images').glob('q[0-9]*_*'):
        # Keep the original image in the local archive before removing the deployed copy.
        assert (OUTPUT / 'images' / image.name).read_bytes() == image.read_bytes()
        image.unlink()
    ...
    print(f"Web: {len(web_bank)} MLA-C01 questions only. MLS content and images stay in output/merged.")
