...
function StudyApp({learner,switchLearner}:{learner:Learner;switchLearner:()=>void}) {
  ...
  return <div className="app-shell">
    ...
    <main id="main" tabIndex={-1}>
      ...
      {route==='/'?<Home state={state} bank={bank} go={go} unfinished={repo.unfinished().filter(s=>isStudySession(s,bank))} resume={id=>{repo.resume(id);go('/session');}} quick={()=>start({...defaultSettings,count:10,quick:true,range:importedBankRange},'practice')}/>:
       ...
       route==='/practice'||route==='/exam'?<Setup key={routeWithQuery} initialFocused={new URLSearchParams(query).get('set')==='focused'} mode={route==='/exam'?'exam':'practice'} bank={bank} state={state} start={start}/>:
       ...
       result?<Results key={result.id} session={result} bank={bank} state={state} go={go} retry={()=>start({...result.settings,scope:'all',count:result.questionIds.length,accuracyMax:100,errorMin:0,minAttempts:1},result.mode,result.questionIds)}/>:
