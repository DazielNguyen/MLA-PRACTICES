...
import { emptyState, formatTime, score, validateState } from './domain';
...
export default function Group({ bank }: { bank: Question[] }) {
  ...
  const view = async (person: SharedLearner) => {
    ...
    try {
      ...
      const validated = validateState({ ...emptyState(), history }, bank).history;
