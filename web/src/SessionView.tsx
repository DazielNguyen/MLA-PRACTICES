import { ArrowLeft, ArrowRight, CheckCircle2, ChevronLeft, EyeOff, Flag, Grid2X2, Send, XCircle, Zap } from 'lucide-react';
...
import { ChoiceList, Dialog, Explanation, IconToggle, QuestionAnswerStats, QuestionText, Tag } from './components';
...
export default function SessionView({session,state,bank,update,go,finish,learnerName,saveLabel}:Props) {
  ...
  const incorrectIds = new Set(immediate ? session.revealed.filter(id=>{
    ...
    return question && session.answers[id]?.length && !isCorrect(question,session.answers[id]);
  }) : []);
  const last = session.index===session.questionIds.length-1;
  const correct = isCorrect(q,answers);
  const FeedbackIcon = correct ? CheckCircle2 : XCircle;
  ...
  useEffect(()=>{
    const key=(e:KeyboardEvent)=>{
      ...
      if(document.querySelector('dialog[open]')||e.altKey||e.ctrlKey||e.metaKey||target?.closest('.assistant-dialog,input,textarea,select,[contenteditable="true"]'))return;
    ...
    <div className="session-layout">
      <main key={quick?q.id:'question'} className={`panel question-panel ${quick?'quick-question':''}`} data-direction={direction}>
        ...
        {revealed&&<div className={quick?'quick-feedback':''}>
          <div role="status" className={`feedback-banner ${correct?'success':'error'}`}>{quick&&<FeedbackIcon size={18}/>}<strong>{(q.conditionalAnswer || q.status==='source' || q.status==='review')?(correct?'Đúng theo đáp án bộ đề.':'Sai theo đáp án bộ đề.'):correct?'Chính xác! Thêm một kiến thức đã nắm vững.':'Chưa chính xác. Cùng xem lại cách giải nhé.'}</strong>{quick&&<span>Bấm lại đáp án hoặc Enter / Space để {last?'xem kết quả':'tiếp tục'}.</span>}</div>
...
export function Results({session,bank,go,state,retry}: {session:Session;bank:Question[];go:(path:string)=>void;state?:State;retry?:()=>void}) {
  const [filter,setFilter]=useState('all');
  const [expanded,setExpanded]=useState<number>();
  const result=score(session,bank), questions=session.questionIds.map(id=>bank.find(q=>q.id===id)!);
  const wrong=questions.filter(q=>!isCorrect(q,session.answers[q.id])).length;
  const shown=questions.filter(q=>filter==='all'||filter==='wrong'&&!isCorrect(q,session.answers[q.id])||filter==='flagged'&&session.flagged.includes(q.id));
  const currentQuestion=shown.find(q=>q.id===expanded);
  useAssistantTopic(currentQuestion ? {kind:'question',id:currentQuestion.id,label:`Câu #${currentQuestion.id} · Xem lại bài làm`,study:{page:'results',selected:session.answers[currentQuestion.id]||[],revealed:true}} : undefined);
  return <div className="page results-page"><div className="page-heading"><div className="eyebrow">SESSION COMPLETE</div><h1>Một bước tiến nữa. Làm tốt lắm!</h1><p>{session.finishReason==='timeout'?'Hết thời gian. Bài làm đã được tự động nộp và lưu.':'Bài làm đã được lưu. Dành vài phút xem lại để nhớ lâu hơn.'}</p></div><section className="results-summary panel"><div className="score-ring" style={{background:`conic-gradient(var(--green) ${result.percent}%, #e8efeb 0)`}}><div><strong>{result.total?`${result.percent}%`:'—'}</strong><span>TỶ LỆ ĐÚNG</span></div></div><div className="result-numbers"><div><strong>{result.correct}<small> / {result.total}</small></strong><span>Câu trả lời đúng</span></div><div><strong>{wrong}</strong><span>Sai hoặc bỏ trống</span></div><div><strong>{formatTime(session.finishedAt!-session.startedAt)}</strong><span>Thời gian làm bài</span></div></div><div className="result-actions">{retry&&<button className="button primary" onClick={retry}>{session.mode==='exam'?'Thi lại bộ này':'Làm lại bộ này'}<ArrowRight size={17}/></button>}<button className="button secondary" onClick={()=>go(session.mode==='exam'?'/exam':'/practice')}>Tạo phiên mới<ArrowRight size={17}/></button></div></section><p className="result-note">Mọi câu đều được tính điểm theo khóa đáp án của bộ đề. Chọn nhiều phải khớp toàn bộ. Đây là tỷ lệ đúng của bài luyện, không quy đổi thành điểm AWS.</p><div className="section-heading"><h2>Xem lại bài làm</h2><div className="filter-tabs">{[['all',`Tất cả (${questions.length})`],['wrong',`Cần ôn lại (${wrong})`],['flagged',`Đánh dấu (${session.flagged.length})`]].map(([value,label])=><button className={filter===value?'active':''} key={value} onClick={()=>setFilter(value)}>{label}</button>)}</div></div><div className="review-list">{shown.length?shown.map(q=><details className="panel review-card" key={q.id} open={expanded===q.id} onToggle={e=>{if(e.target!==e.currentTarget)return;const open=e.currentTarget.open;setExpanded(previous=>open?q.id:previous===q.id?undefined:previous);}}><summary><span className={`review-status ${isCorrect(q,session.answers[q.id])?'success':'error'}`}>{isCorrect(q,session.answers[q.id])?'✓':'×'}</span><span><small>Câu {questions.indexOf(q)+1} · Ngân hàng #{q.id}</small><strong>{q.text}</strong>{state&&<QuestionAnswerStats question={q} state={state}/>}</span><ArrowRight size={16}/></summary><div className="review-body"><QuestionText question={q} allowHint={session.mode==='practice' && session.settings.feedback==='immediate'}/><ChoiceList question={q} selected={session.answers[q.id]||[]} disabled revealed/><Explanation question={q} selected={session.answers[q.id]||[]}/></div></details>):<div className="panel empty"><CheckCircle2 size={30}/><h3>Không có câu trong nhóm này</h3><p>Chọn “Tất cả” để xem toàn bộ bài làm.</p></div>}</div></div>;
}
