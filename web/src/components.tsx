...
export function QuestionText({ question, allowHint = false }: { question: Question; allowHint?: boolean }) {
  return <><p className="question-origin">{sourceLabel(question)}{question.sourceIds.length > 1 && ` · Đã gộp ${question.sourceIds.length} bản`}</p>{question.duplicateOf !== undefined && <p className="notice archived-variant">Biến thể đã gộp vào #{String(question.duplicateOf).padStart(3,'0')}. Đây là bản câu hỏi của phiên đã lưu.</p>}<p className="question-text" lang="en">{question.text}</p><QuestionImages question={question} />{allowHint && question.hint && <details className="question-hint"><summary>Gợi ý — mở sau khi tự phân tích</summary><p>{question.hint}</p></details>}{question.notes.some(note => note.includes('unit')) && <p className="notice"><Info size={15} /> Đơn vị dung lượng trong PDF bị lỗi, đã giữ nguyên ghi chú thay vì đoán đơn vị.</p>}</>;
}
export function Explanation({ question, selected, compact = false }: { question: Question; selected?: string[]; compact?: boolean }) {
  ...
  return <div className={`explanation ${unscored ? 'uncertain' : ''} ${compact?'compact-explanation':''}`}>
    ...
    {analysis && compact && !unscored ? <div className="answer-analysis quick-analysis" lang="en">
      <section className="key-concept"><h4>Key Concept</h4><p>{analysis.keyConcept}</p></section>
      <section className="why-correct"><h4>Why this is correct</h4>{question.answer.map(letter=><p key={letter} data-option={letter}><strong>{letter}.</strong> {analysis.options[letter]}</p>)}</section>
      {selected?.some(letter=>!question.answer.includes(letter))&&<section className="your-choice-analysis"><h4>Why your choice is incorrect</h4>{selected.filter(letter=>!question.answer.includes(letter)).map(letter=><p key={letter} data-option={letter}><strong>{letter}.</strong> {analysis.options[letter]}</p>)}</section>}
      <details className="quick-distractors why-incorrect"><summary>Why other options are incorrect · {Object.keys(question.choices).length-question.answer.length}</summary>{optionAnalysis(Object.keys(question.choices).filter(letter=>!question.answer.includes(letter)))}</details>
    </div> : analysis ? <div className="answer-analysis" lang="en"><h3>Explanation and distractor analysis</h3><section className="key-concept"><h4>Key Concept</h4><p>{analysis.keyConcept}</p></section>{unscored ? <section><h4>Option-by-option analysis</h4><p className="analysis-caveat">The question has unresolved assumptions or conflicting answer keys. The analysis below explains each option without treating a disputed answer as confirmed.</p>{optionAnalysis(Object.keys(question.choices))}</section> : <><section className="why-correct"><h4>{question.status === 'source' ? 'Why the source selects this answer' : 'Why this is correct'}</h4>{optionAnalysis(question.answer)}</section><section className="why-incorrect"><h4>Why other options are incorrect</h4>{optionAnalysis(Object.keys(question.choices).filter(letter=>!question.answer.includes(letter)))}</section></>}</div> : <p className="explanation-copy">{question.explanation}</p>}
    ...
    {Object.entries(question.choices).map(([letter, text], index) => {
      ...
      return <div key={letter} className={`choice ${chosen ? 'selected' : ''} ${correct ? 'correct' : ''} ${wrong ? 'wrong' : ''}`}>
        <button type="button" aria-pressed={chosen} disabled={disabled} onClick={() => onSelect?.(letter)} className="choice-button" title={quick&&revealed?'Bấm lại để tiếp tục':undefined}>
          ...
          <span lang="en">{text}</span>{quick&&<kbd className="choice-shortcut" aria-hidden="true">{index+1}</kbd>}<span className="choice-indicator">{wrong ? <X size={16}/> : chosen||correct ? <Check size={16} /> : null}</span>
        </button>
