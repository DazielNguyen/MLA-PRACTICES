...
export function Explanation({ question, selected, compact = false }: { question: Question; selected?: string[]; compact?: boolean }) {
  ...
  const optionAnalysis = (letters: string[]) => <ul className="option-analysis-list">{letters.map(letter=><li key={letter} data-option={letter}><p className="option-analysis-choice"><strong>{letter}.</strong> <StudyText text={question.choices[letter]} highlight={compact}/></p><p><StudyText text={analysis?.options[letter]||''} highlight={compact}/></p></li>)}</ul>;
  return <div className={`explanation ${unscored ? 'uncertain' : ''} ${compact?'compact-explanation':''}`}>
    ...
    {analysis && compact && !unscored ? <div className="answer-analysis quick-analysis" lang="en">
      <section className="key-concept"><h4>Key Concept</h4><p><StudyText text={analysis.keyConcept} highlight={compact}/></p></section>
      <section className="why-correct"><h4>Why this is correct</h4>{question.answer.map(letter=><p key={letter} data-option={letter}><strong>{letter}.</strong> <StudyText text={analysis.options[letter]} highlight={compact}/></p>)}</section>
      {selected?.some(letter=>!question.answer.includes(letter))&&<section className="your-choice-analysis"><h4>Why your choice is incorrect</h4>{selected.filter(letter=>!question.answer.includes(letter)).map(letter=><p key={letter} data-option={letter}><strong>{letter}.</strong> <StudyText text={analysis.options[letter]} highlight={compact}/></p>)}</section>}
