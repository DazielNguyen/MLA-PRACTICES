...
test('the deployed bank, image assets and catalog contain only MLA questions',()=>{
  assert.equal(bank.length,242);assert.ok(bank.every(q=>q.collection==='mla'&&q.id>332));
  ...
  assert.equal(catalog.total,242);assert.equal(catalog.records,242);assert.deepEqual(catalog.collections,{mla:242});
  assert.deepEqual(catalog.statuses,{checked:210,review:32});
});
test('every study entry point rejects retired questions even if given an old combined bank',()=>{
  const combined=[...archive,...bank];
  assert.equal(studyQuestions(combined).length,242);
  ...
  assert.equal(exam.length,210);assert.ok(exam.every(q=>q.collection==='mla'));
  const session=createSession(combined,{...defaultSettings,count:242},'practice');
