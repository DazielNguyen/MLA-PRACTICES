...
import { CHANGE_EVENT, PROFILE_PREFIX, getProfiles, getWriter, notifyChange, ProgressRepository, storageError as getStorageError, tabGet, tabSet } from './sync/local';
...
export function useLearners() {
  ...
  useEffect(() => {
    const changed = (event: Event) => {
      if (event instanceof StorageEvent && event.key && !event.key.startsWith(PROFILE_PREFIX)) return;
      if (event instanceof CustomEvent && event.detail?.profileId) return;
...
export function useProgress(bank: Question[], learner: Learner) {
  ...
  const sync = useCallback(async (force = false) => {
    ...
    if (!pull && !repo.rows().some(r=>r.dirty)) return;
    ...
    try {
      ...
      setCloudStatus(pending ? 'pending' : 'synced'); setLastSynced(Date.now());
      // Edits made during this request may have outlived their debounce timer.
      // Schedule the next batch after completion, even if updatedAt is unchanged.
      if (pending) setSyncRevision(value => value + 1);
    } catch (error) {
      if (!mounted.current) return;
  ...
  useEffect(() => {
    ...
    const changed = (event: Event) => {
      if (event instanceof StorageEvent && event.key && !event.key.startsWith(repo.prefix)) return;
      if (event instanceof CustomEvent && event.detail?.profileId && event.detail.profileId !== repo.learner.id) return;
      refresh();
    };
    window.addEventListener(CHANGE_EVENT, changed); window.addEventListener('storage', changed);
  ...
  }, [state.updatedAt, syncRevision, sync, repo]);
  const update = useCallback((fn: (s: State) => State) => { if (!repo.update(fn,false)) return; refresh(); if (cloudConfigured && repo.rows().some(r=>r.dirty)) setCloudStatus('pending'); }, [repo, refresh]);
