...
test('identical display names retain separate IDs and progress',()=>{
  const a=make(),b=make();assert.notEqual(a.learner.id,b.learner.id);start(a);a.update(s=>({...s,bookmarks:[2]}));assert.deepEqual(b.state().bookmarks,[]);assert.equal(b.state().active,null);assert.equal(getProfiles().filter(p=>p.name==='Duy').length,2);
});
...
test('one graded answer contributes once across check, finish, replay and sync',()=>{
  const repo=make();start(repo);repo.update(s=>({...s,active:{...s.active!,answers:{2:['B']}}}));repo.update(s=>revealAnswer(s,q));repo.update(s=>revealAnswer(s,q));finish(repo);
  assert.equal(repo.state().progress['2'].attempts,1);const backup=repo.backup();repo.importBackup(backup);assert.equal(repo.state().progress['2'].attempts,1);
  for(const row of repo.rows())repo.mergeRemote(row,row);assert.equal(repo.state().progress['2'].attempts,1);
});
test('independent tab grades accumulate without overwriting the other tab',()=>{
  const a=make(),b=new ProgressRepository(a.learner,bank,crypto.randomUUID());
  const sa=start(a),sb=start(b);
  tabSet(a.activeKey,sa.id);a.update(s=>({...s,active:{...s.active!,answers:{2:['B']}}}));finish(a);
  tabSet(b.activeKey,sb.id);b.update(s=>({...s,active:{...s.active!,answers:{2:['A']}}}));finish(b);
  assert.equal(a.state().progress['2'].attempts,2);assert.equal(a.state().progress['2'].correct,1);
});
test('an old network acknowledgement cannot erase an edit made in flight',()=>{
  const repo=make();repo.update(s=>({...s,known:[2]}));const sent=repo.rows()[0];repo.update(s=>({...s,known:[]}));repo.mergeRemote(sent,sent);
...
test('failed writes remain available in memory even when old disk data exists',()=>{
  const repo=make();repo.update(s=>({...s,known:[2]}));const save=localStorage.setItem;
...
test('legacy migration keeps original bytes and does not mix into the next learner',()=>{
  const repo=make();const original={...emptyState(),known:[2],progress:{2:{attempts:5,correct:3,latest:true,lastSeen:100}}};localStorage.setItem('ml-practice:v1',JSON.stringify(original));repo.migrateLegacy();
  assert.deepEqual(repo.state().known,[2]);assert.equal(repo.state().progress['2'].attempts,5);assert.equal(read('ml-practice:v1'),JSON.stringify(original));
...
test('legacy import does not double-count sessions already represented in a backup',()=>{
  const repo=make();start(repo);repo.update(s=>({...s,active:{...s.active!,answers:{2:['B']}}}));finish(repo);const old=repo.state();
  repo.importBackup(old);repo.importBackup(old);assert.equal(repo.state().progress['2'].attempts,1);
});
test('v2 backup roundtrip preserves all attempts, cards, flags and session rows',()=>{
  const a=make();start(a);a.update(s=>({...s,known:[2,3],bookmarks:[5],active:{...s.active!,answers:{2:['B']},flagged:[2]}}));finish(a);
  const b=make();b.importBackup(validateBackup(a.backup(),bank));assert.deepEqual(b.state().known.sort(),[2,3]);assert.equal(b.state().progress['2'].attempts,1);assert.deepEqual(b.state().history[0].flagged,[2]);assert.equal('code' in a.backup(),false);
});
test('invalid synced rows and backups fail validation',()=>{
  ...
  assert.throws(()=>validateRow({key:'attempt:bad:2',kind:'attempt',value:{questionId:2,sessionId:'other',correct:true,lastSeen:1},stamp:1,writer:'test'},bank));
});
