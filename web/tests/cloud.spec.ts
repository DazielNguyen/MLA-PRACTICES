...
test('Supabase adapter retries queued edits and restores a profile on another device',async({page,context,browser})=>{
  ...
  const records=new Map<string,Map<string,any>>();let failWrites=false,signupCount=0,pushDelay=0,pushing=0,maxPushing=0;
  async function mock(context:BrowserContext){await context.route('https://ml-practice-test.supabase.co/**',async route=>{
    ...
    if(method==='ml_push_rows'){
      pushing++;maxPushing=Math.max(maxPushing,pushing);
      try {
        if(pushDelay)await new Promise(resolve=>setTimeout(resolve,pushDelay));
  ...
  await expect.poll(()=>pushing).toBe(1);
  await page.keyboard.press('3');
  await expect(page.locator('.answer-label')).toHaveText('Đáp án đúng: C',{timeout:1000});
  expect(pushing).toBe(1);
  ...
  const sessionId=(await snapshot(page)).active!.id;
  pushDelay=0;
  await expect.poll(()=>[...records.values()][0].get(`session:${sessionId}`)?.value.revealed,{timeout:10000}).toEqual([333,334]);
  expect(maxPushing).toBe(1);
  await page.reload();
  expect((await snapshot(page)).active!.revealed).toEqual([333,334]);
  ...
  await expect(otherPage.getByRole('button',{name:'Đã nắm',exact:true})).toHaveAttribute('aria-pressed','true');
  await expect(otherPage.locator('.kw-feedback')).toBeVisible();
  await other.close();
});
