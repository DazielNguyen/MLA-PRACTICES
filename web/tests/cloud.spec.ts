...
test('Supabase adapter retries queued edits and restores a profile on another device',async({page,context,browser})=>{
  ...
  await expect(page.locator('.answer-label')).toHaveText('Correct answer: B',{timeout:1000});
  ...
  expect((await snapshot(page)).active!.revealed).toEqual([2,3]);
  const sessionId=(await snapshot(page)).active!.id;
  pushDelay=0;
  await expect.poll(()=>[...records.values()][0].get(`session:${sessionId}`)?.value.revealed,{timeout:10000}).toEqual([2,3]);
