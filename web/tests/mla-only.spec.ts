...
test('all web study modes expose only MLA questions and no MLS selector',async({page})=>{
  await onboard(page);
  await expect(page.locator('.bank-card')).toContainText('437 câu hỏi');
  ...
  await page.getByLabel('Số câu hỏi',{exact:true}).fill('437');
  await page.getByRole('button',{name:'Bắt đầu thi thử',exact:true}).click();
  const ids=(await snapshot(page)).active!.questionIds;
  expect(ids).toHaveLength(437);expect(ids.every(id=>id>332)).toBe(true);
  await page.goto('/#/library');
  await expect(page.locator('.library-count')).toContainText('437 câu hỏi');
